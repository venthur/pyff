...
class BrainPong(PygameFeedback):
    ...
    def trial_tick(self):
        ...
        if border1 < 0 or border2 > self.geometry[2]:
        ...
        if self.control == "absolute":
            class_out = self.f
            moveLength = (self.geometry[2]-2*self.wallW-self.barRect.width) / 2
            ...
            barLeft = self.geometry[2]/2+newBarX-self.barRect.width/2
            barRight = self.geometry[2]/2+newBarX+self.barRect.width/2
            if barLeft > self.wallW and barRight < self.geometry[2] - self.wallW:
    ...
    def init_graphics(self):
        ...
        barHeight = self.geometry[3]/15
        try:
            self.oldPlayWidth = self.playWidth
            self.oldBarSurface = self.barSurface
        except:
            pass
        self.barSurface = self.geometry[3]-barHeight * 3/2
        ...
        self.background = pygame.transform.scale(img, (self.geometry[2], self.geometry[3]))
        self.backgroundRect = self.background.get_rect(center=self.screen.get_rect().center)

        # init walls
        self.wallSize = (max(0,(self.geometry[2]-self.barSurface)/2),self.geometry[3])
        ...
        self.wallRect2 = self.wall.get_rect(topleft=(self.geometry[2]-self.wallW,0))
        self.fontsize_target = self.size/16

        # init bar
        barWidth = int((self.geometry[2]-2*self.wallW) * (self.barWidth/100.0))
        ...
        self.barMB = (self.geometry[2]/2, self.barSurface+barHeight)
        ...
        self.bowlRect = self.bowl.get_rect(center=((self.geometry[2]-2*self.wallW)/3+self.wallW, diameter/2))
        self.bowl.set_colorkey((168,180,255))

        # calculate width of playing area
        self.playWidth = self.geometry[2]-2*self.wallW
        ...
        if not self.resized:
            ...
            self.resized = False
            self.counterSize = self.geometry[3]/20
